...
experiment_name = "exp_" * expe * "inst_" * instance * "d_" * string(dim) * "_N" * string(Nruns);
...
function runit(seed, N, instance, d)
	...
	elseif instance == "Laplace"
		...
		# ΔLLE
	    ΔLLE = (2 * σ.(absS) .- 1) .* σ.(-absS);
	    meanΔLLE = mean(ΔLLE);
	    stdΔLLE = std(ΔLLE);

	    meanRLLE, stdRLLE = 0., 0.;
	else
...
data = runit(seed, Nruns, instance, dim);

# Save everything using JLD2.
@save "$(experiment_dir)$(experiment_name).dat" data;
meanΔSP, stdΔSP, meanΔLLE, stdΔLLE, meanRLLE, stdRLLE = data;
...
for io in ios
    println(io, "");
	println(io, "Mean ΔSP = ", meanΔSP);
	if instance == "Gaussian" && dim > 1
		println(io, "λmin ΔSP = ", stdΔSP);
	end
    println(io, "");
	println(io, "Mean ΔLLE = ", meanΔLLE);
	if instance == "Gaussian" && dim > 1
		println(io, "λmin ΔLLE = ", stdΔLLE);
	end
    println(io, "");
	println(io, "Mean RLLE = ", meanRLLE);
	if instance == "Gaussian" && dim > 1
		println(io, "λmin RLLE = ", stdRLLE);
	end
end
