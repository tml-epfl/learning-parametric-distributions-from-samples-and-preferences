...
# Loading
experiments = filter(x -> isdir("$(exp_dir)$x") && occursin("_error1d_", x) && occursin("Laplace", x), readdir(exp_dir));
for experiment in experiments
    ...
    if ! existing_plots
        ...
        AEs = permutedims(hcat(getindex.(data, 4)...));
        ...
        meanAEs = mean(abs.(AEs), dims=1)[1,:];
        stdAEs = std(abs.(AEs), dims=1)[1,:];
        ...
        # Plot Loss
        plot([u*batch for u in 1:Int(sizemax)], meanSOMLEs, ribbon=stdSOMLEs, fillalpha=.3, label="SO", linestyle=:auto, color=dict_colors["SO"], linewidth=2, legend=:topright, xlabel=L"n", ylabel=L"|\widehat \theta_n - \theta^\star|", guidefontsize=18, tickfontsize=16, legendfontsize=12);
        ...
        file_plot = "$(exp_dir)$(experiment)/plot_error_laplace_loss.$format";
        savefig(file_plot);

        # Plot Inv loss
        plot([u*batch for u in 1:Int(sizemax)], inv_meanWEs, fillalpha=.3, label="WE", linestyle=:auto, color=dict_colors["WE"], linewidth=2, legend=:topleft, xlabel=L"n", ylabel=L"1/|\widehat \theta_n - \theta^\star|", guidefontsize=18, tickfontsize=16, legendfontsize=12);
        ...
        file_plot = "$(exp_dir)$(experiment)/plot_error_laplace_invloss.$format";
        savefig(file_plot);

        # Log-Log Plot Loss
        plot([u*batch for u in 1:Int(sizemax)], meanSOMLEs, label="SO", linestyle=:auto, color=dict_colors["SO"], linewidth=2, legend=:bottomleft, yaxis=:log, xaxis=:log, xlabel=L"n", ylabel=L"|\widehat \theta_n - \theta^\star|", guidefontsize=18, tickfontsize=16, legendfontsize=12);
        ...
        file_plot = "$(exp_dir)$(experiment)/plot_error_laplace_logplot_loss.$format";
        savefig(file_plot);
