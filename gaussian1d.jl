	...
	if instance == "Gaussian"
        ...
        for i in 1:sizemax
            ...
            @objective(model1, Min, (x1 - SOMLEs[Nloc])^2 + 
                                    ...
                                    #sum(log.(1 .+ exp.(-Z[1:Nloc] .* (X[1:Nloc] .- Y[1:Nloc]) .* (x1 .- S[1:Nloc])))) / Nloc);
        ...
        for i in 1:sizemax
            ...
            # Objective
            V1 = Zsto[1:Nloc] .* (X[1:Nloc] .- Y[1:Nloc]);
            V2 = Zsto[1:Nloc] .* (X[1:Nloc] .- Y[1:Nloc]) .* S[1:Nloc];
            @objective(model2, Min, (x2 - SOMLEs[Nloc])^2 + 
                                    sum(log.(1 .+ exp.(V2 .- x2 * V1))) / Nloc);
        ...
        return μ, SOMLEs .- μ, WEs .- μ, RUs .- μ, DPMLEs .- μ, LLEs .- μ, SPMLEs .- μ;
	else
