...
# Loading
experiments = filter(x -> isdir("$(exp_dir)$x") && occursin("_error1d_", x) && occursin("Rayleigh", x), readdir(exp_dir));
for experiment in experiments
    ...
    if ! existing_plots
        ...
        file_plot = "$(exp_dir)$(experiment)/plot_error_rayleigh_loss.$format";
        savefig(file_plot);
        ...
        file_plot = "$(exp_dir)$(experiment)/plot_error_rayleigh_invloss.$format";
        savefig(file_plot);
        ...
        file_plot = "$(exp_dir)$(experiment)/plot_error_rayleigh_logplot_loss.$format";
        savefig(file_plot);
